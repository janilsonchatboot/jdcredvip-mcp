...
export async function simularCrefaz(proposta, actor = "manual") {
  ...
  await logIntegration("crefaz", "simulacao", "sucesso", `Proposta ${id} simulada`, { actor, propostaId: id });
...
export async function contratarCrefaz(payload, actor = "manual") {
  ...
  await logIntegration("crefaz", "contratacao", "sucesso", `Proposta ${propostaId} atualizada para ${status}`, {
    actor,
    propostaId,
    status
  });
