...
import { WebSocketMessage, InsertMessage, InsertCustomer, InsertConversation, Customer, Conversation } from "@shared/schema";
...
export class WhatsAppService {
   ...
   */
  /**
   * Método para broadcast de mensagens para os clientes WebSocket conectados
  ...
  private async handleAutomaticReply(conversationId: number, messageContent: string): Promise<void> {
    try {
      ...
      const formatMessages = messages.slice(-10).map(msg => ({
  ...
  private async createLeadFromConversation(
    ...
    try {
      ...
      const newLead = await storage.createLead({
        ...
        status: "active",
        conversationId: conversation.id,
        priority: leadInfo.urgency === "high" ? "alta" : 
                 leadInfo.urgency === "medium" ? "média" : "baixa",
...
Observações: ${leadInfo.notes || 'N/A'}`;
      ...
      await storage.createLeadActivity({
        ...
        content: noteContent,
        createdAt: new Date(),
        createdById: 1, // ID do sistema/bot
        dueDate: null,
        completedAt: null
      });
