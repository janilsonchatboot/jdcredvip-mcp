  ...
  async exportConversationsToCSV(outputPath: string = 'exports/conversations.csv'): Promise<string> {
    try {
      ...
      for (const conversation of conversations) {
        ...
        const topics = this.identifyTopics(messages.map(msg => msg.content));
        
        const row = [
          conversation.id,
          this.escapeCSV(conversation.customer.name),
          this.escapeCSV(conversation.customer.phoneNumber),
          conversation.status,
          conversation.channel,
          conversation.createdAt instanceof Date ? conversation.createdAt.toISOString() : conversation.createdAt,
          conversation.updatedAt instanceof Date ? conversation.updatedAt.toISOString() : conversation.updatedAt,
          customerMessages,
          systemMessages,
          this.escapeCSV(topics.join(', '))
        ];
