  ...
  app.patch('/api/leads/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (result.data.status) {
        if (result.data.status === 'won' && !result.data.wonAt) {
          result.data.wonAt = new Date().toISOString();
        } else if (result.data.status === 'lost' && !result.data.lostAt) {
          result.data.lostAt = new Date().toISOString();
        }
      }
      
      const updatedLead = await storage.updateLead(id, result.data);
